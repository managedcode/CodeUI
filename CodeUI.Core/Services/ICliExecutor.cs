...
public interface ICliExecutor : IDisposable
{
    ...
    /// <param name="arguments">The arguments to pass to the command.</param>
    /// <param name="workingDirectory">The working directory for the process. Defaults to current directory.</param>
    /// <param name="terminalSize">The initial terminal size (columns, rows). Defaults to 80x24.</param>
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    /// <returns>A task that completes when the process starts, containing the process information.</returns>
    Task<ProcessInfo> StartPtyProcessAsync(string command, string arguments, string? workingDirectory = null, 
    ...
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    /// <returns>A task that completes when the resize is applied.</returns>
    ...
    /// </summary>
    /// <param name="signal">The signal to send (e.g., SIGINT for Ctrl+C).</param>
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    /// <returns>A task that completes when the signal is sent.</returns>
